function validateForm() {
    ...
    // Check if the zip code is exactly 5 digits if filled out
    const zipCodePattern = /^\d{5}$/;
    if (!zipCodePattern.test(zipCode)) {
        // Displays a warning message if the zip code is invalid
        warningMessage.textContent = 'Error: Zip code must be exactly 5 digits.';
        return false; // Stop form processing
    }
